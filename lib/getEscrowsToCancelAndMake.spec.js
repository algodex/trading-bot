...
const escrows = [
  { doc: { order: { price: 12.3, type: "buy" }, _id: "ESCROWADDR_BUY1" } },
  { doc: { order: { price: 12.1, type: "buy" }, _id: "ESCROWADDR_BUY2" } },
  { doc: { order: { price: 11.9, type: "buy" }, _id: "ESCROWADDR_BUY3" } },
  { doc: { order: { price: 12.5, type: "sell" }, _id: "ESCROWADDR_SELL1" } },
  { doc: { order: { price: 12.7, type: "sell" }, _id: "ESCROWADDR_SELL2" } },
  { doc: { order: { price: 12.9, type: "sell" }, _id: "ESCROWADDR_SELL3" } },
];
...
test("all orders cancel", () => {
  ...
  const escrowChanges = getEscrowsToCancelAndMake({
    escrows,
    latestPrice,
    minSpreadPerc,
    nearestNeighborKeep,
...
test("some orders cancel A", () => {
  ...
  const nearestNeighborKeep = 0.0025;
  const idealPrices = [
    12.06832088196718, 12.109117906194292, 12.15944477122892,
...
test("some orders cancel B", () => {
  ...
  const idealPrices = [
    11.909662227031992, 11.977319665085368, 12.054716226914762,
    12.126196078424046, 12.272588787620313, 12.347532672313463,
    12.416363820085335, 12.495275320860225,
  ];
  const escrowChanges = getEscrowsToCancelAndMake({
    escrows,
    latestPrice,
